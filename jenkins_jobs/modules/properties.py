...
def sidebar(parser, xml_parent, data):
    ...
    Requires the Jenkins `Sidebar Plugin.
    <https://wiki.jenkins-ci.org/display/JENKINS/Sidebar+Plugin>`_
    ...
    XML.SubElement(action, 'url').text = str(
        data.get('url', ''))
    XML.SubElement(action, 'text').text = str(
        data.get('text', ''))
    XML.SubElement(action, 'icon').text = str(
        data.get('icon', ''))
